...
export function useGrainGradient(options: GrainGradientCSSOptions = {}) {
  const meshKey = JSON.stringify([options.colors, options.baseColor, options.intensity, options.blur]);
  ...
  const meshStyle = useMemo(() => ({ backgroundColor: options.baseColor ?? "#0b1020", backgroundImage: meshCss, filter: `blur(${Math.max(0, Math.min(80, options.blur ?? 24))}px) saturate(${Math.max(0.2, Math.min(2.5, options.intensity ?? 1))})` }), [meshCss, options.baseColor, options.blur, options.intensity]);
